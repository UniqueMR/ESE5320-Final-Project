...
static void lzw(hls::stream<unsigned char>& input_stream, hls::stream<int>& length, hls::stream<unsigned char>& output_stream, hls::stream<int>& total_bytes){
    ...
    int total_bits = out_len * 12;
    int total_bytes_val = (total_bits + 7) / 8;;
    uint32_t header = static_cast<uint32_t>(total_bytes_val) << 1;
    total_bytes.write(total_bytes_val);

    for (int i = 0; i < 4; ++i) {
        #pragma HLS UNROLL
        output_stream.write(header >> (i * 8));
    }
...
void lzw_multi_chunks(unsigned char multi_chunks[CHUNKS_IN_SINGLE_KERNEL * MAX_CHUNK_SIZE], 
                      int length[CHUNKS_IN_SINGLE_KERNEL], 
                      unsigned char file_buffer[CHUNKS_IN_SINGLE_KERNEL * MAX_FILE_BUFFER_SIZE], 
                      int total_bytes[CHUNKS_IN_SINGLE_KERNEL]){
    ...
    #pragma HLS stream variable=stream_input depth=25000
    #pragma HLS stream variable=stream_output depth=37504
    #pragma HLS stream variable=len_input depth=1
    #pragma HLS stream variable=len_output depth=1
    ...
    // Process each chunk with LZW compression
    Main_loop_i: for(int i = 0; i < CHUNKS_IN_SINGLE_KERNEL; i++){
        // Assuming lzw is modified to work with streams
...
//         lzw(input_buffer[i], length[i], &file_buffer[i * MAX_FILE_BUFFER_SIZE], total_bytes + i);
//     }
