  ...
  proc write_debug_ip_layout { {is_hw_emu false} {hw_platform_vbnv "none"} {vpl_output_dir "."} } {
    ...
    foreach noc $nocs {
      ...
      foreach nocPin $nocPins {
        ...
        set npiClockFreqMhz 299.997009
        set cips [get_bd_cells -quiet -filter {VLNV=~"*versal_cips*"}]
        if {$cips != {}} {
          set npiClockFreqMhz [get_property -quiet CONFIG.PMC_CRP_NPI_REF_CTRL_ACT_FREQMHZ $cips]
        }

        #set aieClockFreqMhz 1000
        #set aie [get_bd_cells -quiet -filter {VLNV=~"*ai_engine*"}]
        #if {$aie != {}} {
        #  set aieClockFreqMhz [get_property -quiet CONFIG.AIE_CORE_REF_CTRL_FREQMHZ $aie]
        #}

        # Use format: <master pin>-<slave pin>
        # <master pin> will be the pretty name used in reporting
        # <slave pin> will be used in  
