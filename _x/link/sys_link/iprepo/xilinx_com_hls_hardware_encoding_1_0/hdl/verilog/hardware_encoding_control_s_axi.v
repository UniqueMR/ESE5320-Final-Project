...
#(parameter
    ...
    output wire [63:0]                   out_code,
    output wire [63:0]                   header,
    output wire [63:0]                   out_len,
...
localparam
    ...
    ADDR_S1_CTRL         = 7'h18,
    ADDR_LENGTH_R_DATA_0 = 7'h1c,
    ADDR_LENGTH_R_CTRL   = 7'h20,
    ADDR_OUT_CODE_DATA_0 = 7'h24,
    ADDR_OUT_CODE_DATA_1 = 7'h28,
    ADDR_OUT_CODE_CTRL   = 7'h2c,
    ADDR_HEADER_DATA_0   = 7'h30,
    ADDR_HEADER_DATA_1   = 7'h34,
    ADDR_HEADER_CTRL     = 7'h38,
    ADDR_OUT_LEN_DATA_0  = 7'h3c,
    ADDR_OUT_LEN_DATA_1  = 7'h40,
    ADDR_OUT_LEN_CTRL    = 7'h44,
    WRIDLE               = 2'd0,
    WRDATA               = 2'd1,
    WRRESP               = 2'd2,
    WRRESET              = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_out_code = 'b0;
    reg  [63:0]                   int_header = 'b0;
    reg  [63:0]                   int_out_len = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_OUT_CODE_DATA_0: begin
                    rdata <= int_out_code[31:0];
                end
                ADDR_OUT_CODE_DATA_1: begin
                    rdata <= int_out_code[63:32];
                end
                ADDR_HEADER_DATA_0: begin
                    rdata <= int_header[31:0];
                end
                ADDR_HEADER_DATA_1: begin
                    rdata <= int_header[63:32];
                end
                ADDR_OUT_LEN_DATA_0: begin
                    rdata <= int_out_len[31:0];
                end
                ADDR_OUT_LEN_DATA_1: begin
                    rdata <= int_out_len[63:32];
                end
            endcase
        end
    end
...
assign out_code    = int_out_code;
assign header      = int_header;
assign out_len     = int_out_len;
// int_event_start
always @(posedge ACLK) begin
    if (ARESET)
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_OUT_CODE_DATA_0)
            int_out_code[31:0] <= (WDATA[31:0] & wmask) | (int_out_code[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_out_code[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUT_CODE_DATA_1)
            int_out_code[63:32] <= (WDATA[31:0] & wmask) | (int_out_code[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_HEADER_DATA_0)
            int_header[31:0] <= (WDATA[31:0] & wmask) | (int_header[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_HEADER_DATA_1)
            int_header[63:32] <= (WDATA[31:0] & wmask) | (int_header[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_OUT_LEN_DATA_0)
            int_out_len[31:0] <= (WDATA[31:0] & wmask) | (int_out_len[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_OUT_LEN_DATA_1)
            int_out_len[63:32] <= (WDATA[31:0] & wmask) | (int_out_len[63:32] & ~wmask);
    end
end
