...
int main(int argc, char *argv[])
{
    ...
    cl::Kernel krnl_mmult(program, "mmult_fpga:{mmult_fpga_1}", &err);
    cl::Kernel krnl_mmult1(program, "mmult_fpga:{mmult_fpga_2}", &err);
    ...
    std::vector<std::vector<cl::Event>> write_waitlist(2);
    ...
    for (int i = 0; i < NUM_TESTS; i+=2)
    {
        ...
        krnl_mmult1.setArg(0, a_buf[(i+1)%NUM_MAT]);
        krnl_mmult1.setArg(1, b_buf[(i+1)%NUM_MAT]);
        krnl_mmult1.setArg(2, c_buf[(i+1)%NUM_MAT]);
        ...
        {   
            ...
            write_waitlist[0].push_back(write_done[i]);
            q.enqueueMigrateMemObjects({a_buf[(i+1)%NUM_MAT], b_buf[(i+1)%NUM_MAT]}, 0 /* 0 means from host*/, NULL, &write_done[i+1]);
            write_waitlist[1].push_back(write_done[i+1]);
        }
        else
        {
            // q.enqueueMigrateMemObjects({a_buf[i%NUM_MAT], b_buf[i%NUM_MAT]}, 0 /* 0 means from host*/, &read_events, &write_ev);
            q.enqueueMigrateMemObjects({a_buf[i%NUM_MAT], b_buf[i%NUM_MAT]}, 0 /* 0 means from host*/, &write_waitlist[0], &write_done[i]);
            write_waitlist[0].push_back(write_done[i]);
            q.enqueueMigrateMemObjects({a_buf[(i+1)%NUM_MAT], b_buf[(i+1)%NUM_MAT]}, 0 /* 0 means from host*/, &write_waitlist[1], &write_done[i+1]);
            write_waitlist[1].push_back(write_done[i+1]);
        ...
        execute_waitlists[i+1].push_back(write_done[i+1]);
        q.enqueueTask(krnl_mmult1, &execute_waitlists[i+1], &execute_done[i+1]);
        ...
        read_waitlists[i+1].push_back(execute_done[i+1]);
        q.enqueueMigrateMemObjects({c_buf[(i+1)%NUM_MAT]}, CL_MIGRATE_MEM_OBJECT_HOST, &read_waitlists[i+1], &read_done[i+1]);
    }
